...
of 2: 
\begin{theorem}[Hoeffding inequality, two-sided]
\label{thm:2.2.5}
...
\label{rmk:2.3.3}
When $p_i$ is small for the Bernoulli random variables, by the Poisson Limit Theorem (\cref{thm:1.7.6}), 
...
\begin{theorem}[Median-of-means estimator]
\label{thm:2.4.1}
...
\begin{proposition}[Dense graphs are almost regular]
\label{prop:2.5.1}
...
\begin{remark}[Sparse random graphs are far from regular]
...
$\color{blue} (a) \Rightarrow (b)$ Assume $(a)$ holds. Without loss of generality, assume $K_1 = 1$ since it 
only affects the other constants we obtain by a constant factor, so we can just scale everything.
The integrated tail formula (\cref{lem:1.6.1}) for $|X|^p$ gives 
\begin{align*}
	\mathbb{E}[|X|^p] 
...
$(b)$ guarantees that $\mathbb{E}[X^{2p}] \leq (2p)^p$, and $p! \geq (p / e)^p$ by \cref{lem:1.7.8}, 
...
\[ \left\lVert \sum_{i = 1}^{N} X_i \right\lVert_{\psi_2}^2 \leq C\sum_{i = 1}^{N} 
\lVert X_i \rVert_{\psi_2}^2, \]
...
independent. Then 
\[ \lVert \max_{i = 1, \dots, N} X_i \rVert_{\psi_2} \leq C \sqrt{\ln{N}} 
